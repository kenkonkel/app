...
namespace app.web.application.stubs
{
  public class StubDepartmentFinder:IDepartmentFinder
  {
    public IEnumerable<Department> get_the_main_departments()
    {
  	...
  	public IEnumerable<Department> get_the_departments_for_department(Department department)
  	{
		return Enumerable.Range(1, 25).Select(x => new Department { name = x.ToString("Sub-Department 0") });
	}
  }
}